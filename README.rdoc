...
* Install php
  sudo apt-get install php5-cgi php5-cli php5-dev php-pear php-fpm
...
* Edit /etc/php5/fpm/php.ini so that cgi.fix_pathinfo=1 is enabled

* Edit /etc/php5/fpm/pool.d/www.conf so that listen = /var/run/php-fpm/php-fpm.sock (create that folder too)
...
* Setup the nginx config
    ...
    sudo cp /srv/rutorrent.tigris.id.au/config/nginx/rutorrent.tigris.id.au .
